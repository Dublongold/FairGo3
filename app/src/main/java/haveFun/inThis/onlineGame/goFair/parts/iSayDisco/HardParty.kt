...
import haveFun.inThis.onlineGame.goFair.parts.iSayDisco.gameLogics.HardGameLogic
...
class HardParty: Fragment() {
    ...
    private lateinit var partyElement7: View
    private lateinit var partyElement8: View
    private lateinit var partyElement9: View

    private lateinit var gameLogic: HardGameLogic

    override fun onCreateView(
        inflater: LayoutInflater,
        container: ViewGroup?,
        savedInstanceState: Bundle?
    ): View? = inflater doInflate (R.layout.hard_party to container)

    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)
        val partyNumber = partyNumberContainer.partyNumber
        gameLogic = HardGameLogic(partyNumber)
        view.doInside {
            ...
            partyElement7 = findViewById(R.id.partyElement7)
            partyElement8 = findViewById(R.id.partyElement8)
            partyElement9 = findViewById(R.id.partyElement9)
        ...
        partyElement7.setOnClickListener {
            partyElementClick(it, 6)
        }
        partyElement8.setOnClickListener {
            partyElementClick(it, 7)
        }
        partyElement9.setOnClickListener {
            partyElementClick(it, 8)
        }
        gameLogic.canClick.observe(viewLifecycleOwner) {
            ...
            partyElement7.isEnabled = it
            partyElement8.isEnabled = it
            partyElement9.isEnabled = it
    ...
    private fun failed() {
        viewLifecycleOwner.lifecycleScope.launch {
            ...
            partyElement5.setBackgroundResource(R.drawable.element_hidden)
            partyElement6.setBackgroundResource(R.drawable.element_hidden)
            partyElement7.setBackgroundResource(R.drawable.element_hidden)
            partyElement8.setBackgroundResource(R.drawable.element_hidden)
            partyElement9.setBackgroundResource(R.drawable.element_hidden)
        ...
        view?.doInside {
            ...
            findViewById<View>(R.id.nextButton).apply {
                if(partyNumber != 20) {
                    setOnClickListener {
                        PossibleDestinations.POP_BACK_STACK navigate null
                        PossibleDestinations.HARD_PARTY navigate partyNumber + 1
                    }
                }
                else {
